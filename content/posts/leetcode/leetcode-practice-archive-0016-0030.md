...
- 最朴素的做法是顺次合并两个相邻的有序链表, 但是这样做的话每次合并得到的新链表长度均为两个旧链表的长度之和, 累积效应显著 (时间复杂度为 $O(k^2n)$). 优化的话可以考虑 Huffman Tree (或等价的 2 路归并), 每一轮合并的时间复杂度均为 $O(kn)$, 共合并 $O(\log(k))$ 轮, 因此总的时间复杂度仍为 $O(kn\log(k))$.
